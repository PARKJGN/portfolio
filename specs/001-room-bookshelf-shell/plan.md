...
**Language/Version**: TypeScript 5.5 · Node.js 20.9+ (빌드 전용) · React 18.3

**Primary Dependencies**: Next.js 16 (App Router, `output: 'export'`) · Tailwind CSS 3.4 · Zod (콘텐츠 검증)
...
**Constraints**: WCAG 2.2 AA · 320px 이상 가로 스크롤 없음 · 폰트 self-host + 서브셋 · arm64 이미지 필수
(JS 용량 상한은 헌장 v2.0.0에서 삭제됨 — R-1 참조)
...
| 원칙 | 판정 | 근거 |
|---|---|---|
| I. Content-First, Static by Default | **통과** | `output: 'export'`로 런타임 서버 없음. 책 내용은 서버 렌더 HTML 로 `<dialog>` 안에 존재해 낭독기·검색이 읽는다. 열람 조작(3D/모달)은 JS 를 전제로 한다(헌장 3.0.0 에서 무JS 열람 조항 삭제). |
...
| III. Performance Budgets Are Gates | **통과** | 실측 결과 Next 16 기준 gzip 145.8KB로 기존 100KB 상한을 넘겼고, 이를 근거로 **헌장 v2.0.0에서 용량 상한 조항을 삭제**했다(R-1). 남은 게이트는 LCP·CLS·INP이며 설계상 이를 위협하는 요소는 없다. |
...
**게이트 결과**: 다섯 원칙 모두 통과. 원칙 III은 실측(R-1) 후 헌장 v2.0.0에서 용량 조항이
삭제되면서 해소되었다. 추측이 아니라 측정된 숫자를 근거로 개정했다.
...
│   └── page.tsx                    # 방 (책장 3개) + 모든 책 <dialog> 프리렌더
├── components/
│   ├── room/                       # 방 배경, 책장, 책등, 가로 스와이프
│   └── book/                       # 책 창, 3D 리더, 보기 모드 전환, 장 이동
├── content/
│   ├── shelves/*.md                # 책장 정의
...
|-----------|------------|-------------------------------------|
| Next.js 채택 — 원칙 III 예산을 위협 | 사이트 주인이 직접 유지보수하며, oneBite에서 이미 Next.js + Tailwind + TypeScript로 같은 배포 파이프라인을 운영 중이다(oneBite는 14, 이 프로젝트는 16). 익숙한 스택이 장기 유지보수 가능성을 높인다. | Astro나 순수 HTML이 예산 면에서 유리하지만, 익숙하지 않은 스택은 사이트가 방치될 위험을 키운다. 대신 정적 export와 클라이언트 컴포넌트 최소화로 위험을 줄이고 R-1에서 실측해 판단한다. |
...
| 원칙 | 판정 | 설계가 바꾼 점 |
|---|---|---|
| I | **통과 (헌장 3.0.0 후)** | 모든 책 내용을 방 페이지의 `<dialog>` 안에 서버 렌더 HTML 로 프리렌더해 낭독기·검색이 읽는다. 열람 조작(3D/모달)은 JS 를 전제로 하며, 이를 위한 `/books/[slug]` 정적 페이지는 삭제했다(헌장 3.0.0). |
...
| III | **통과 (헌장 개정 후)** | 실측값이 상한을 넘겨 헌장 v2.0.0에서 용량 조항을 삭제했다. 설계 자체는 바뀌지 않았다 — 클라이언트 컴포넌트는 책 창 하나뿐이고 장 나눔은 CSS가 담당해 JS 표면은 여전히 최소다. 이제 게이트는 LCP·CLS·INP뿐이다. |
